...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [prods, cats, arts] = await Promise.all([
    db.select({ slug: products.slug, updatedAt: products.updatedAt }).from(products).where(and(eq(products.organizationId, org.id), inArray(products.status, PUBLIC_PRODUCT_STATUSES as Product["status"][]), visibleOnStorefront(), demo ? undefined : eq(products.isDemo, false))),
