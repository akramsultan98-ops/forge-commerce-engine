...
export function ProductForm({ action, product, categories, submitLabel }: { action: Action; product?: Product; categories: Category[]; submitLabel: string }) {
  const p = product;
  return (
    <ActionForm action={action} className="space-y-8">
      ...
      <fieldset className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="Title" htmlFor="title" className="md:col-span-2">
          <Input id="title" name="title" required minLength={3} maxLength={200} defaultValue={p?.title} />
        </Field>
        <Field label="Problem it solves" htmlFor="problemSolved" hint="Phrase it so it reads after “fixes …”, e.g. “charging cables falling behind the desk”.">
          <Input id="problemSolved" name="problemSolved" maxLength={300} defaultValue={p?.problemSolved ?? ""} />
        </Field>
        <Field label="Target audience" htmlFor="targetAudience">
          <Input id="targetAudience" name="targetAudience" maxLength={300} defaultValue={p?.targetAudience ?? ""} />
        </Field>
        <Field label="Description" htmlFor="description" className="md:col-span-2">
          <Textarea id="description" name="description" rows={3} maxLength={5000} defaultValue={p?.description ?? ""} />
        </Field>
        <Field label="Verified highlights (one per line)" htmlFor="highlights" hint="Only facts you can back up — copy generation uses these." className="md:col-span-2">
          <Textarea id="highlights" name="highlights" rows={3} defaultValue={p?.highlights.join("\n") ?? ""} />
        </Field>
        <Field label="Category" htmlFor="categoryId">
          <Select id="categoryId" name="categoryId" defaultValue={p?.categoryId ?? ""} options={[{ value: "", label: "—" }, ...categories.map((c) => ({ value: c.id, label: c.name }))]} />
        </Field>
        <Field label="Brand" htmlFor="brand">
          <Input id="brand" name="brand" maxLength={120} defaultValue={p?.brand ?? ""} />
        </Field>
      ...
      <fieldset className="grid gap-4 md:grid-cols-4">
        ...
        <Field label="Currency" htmlFor="currency">
          <Select id="currency" name="currency" defaultValue={p?.currency ?? "USD"} options={CURRENCIES.map((c) => ({ value: c, label: c }))} />
        </Field>
        <Field label="Selling price" htmlFor="sellingPrice">
          <Input id="sellingPrice" name="sellingPrice" inputMode="decimal" defaultValue={num(p?.sellingPrice)} />
        </Field>
        ...
        <Field label="Commission %" htmlFor="commissionPercentage">
          <Input id="commissionPercentage" name="commissionPercentage" inputMode="decimal" defaultValue={num(p?.commissionPercentage)} />
        </Field>
      ...
      <fieldset className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="Affiliate URL" htmlFor="affiliateUrl">
          <Input id="affiliateUrl" name="affiliateUrl" type="url" defaultValue={p?.affiliateUrl ?? ""} placeholder="https://…" />
        </Field>
        <Field label="Product / store URL" htmlFor="productUrl">
          <Input id="productUrl" name="productUrl" type="url" defaultValue={p?.productUrl ?? ""} placeholder="https://…" />
        </Field>
        ...
        <Field label="Image URL" htmlFor="imageUrl" hint="Use the supplier's or merchant's real product image.">
          <Input id="imageUrl" name="imageUrl" type="url" defaultValue={p?.imageUrl ?? ""} placeholder="https://…" />
        </Field>
      </fieldset>
      ...
      <fieldset className="grid gap-4 md:grid-cols-4">
        ...
        <Field label="Rating (0–5)" htmlFor="rating">
          <Input id="rating" name="rating" inputMode="decimal" defaultValue={num(p?.rating)} />
        </Field>
        <Field label="Review count" htmlFor="reviewCount">
          <Input id="reviewCount" name="reviewCount" inputMode="numeric" defaultValue={num(p?.reviewCount)} />
        </Field>
        ...
        <Field label="Countries (one per line, ISO codes)" htmlFor="countriesAvailable" className="md:col-span-2">
