...
import { affiliateLinks, articles, categories, productReviews, productSignals, products, type Product } from "../db/schema";
...
export async function publicProducts(opts: { sort?: Sort; maxPrice?: number; categoryId?: string; limit?: number; excludeIds?: string[]; statuses?: string[] } = {}): Promise<PublicProduct[]> {
  ...
  const conds: Array<SQL | undefined> = [eq(products.organizationId, org.id), inArray(products.status, (opts.statuses ?? PUBLIC_PRODUCT_STATUSES) as Product["status"][]), eq(products.available, true)];
...
export async function publicProductBySlug(slug: string) {
  ...
  const [row] = await db
    ...
    .from(products)
    .leftJoin(categories, eq(categories.id, products.categoryId))
    .where(and(eq(products.organizationId, org.id), eq(products.slug, slug), inArray(products.status, PUBLIC_PRODUCT_STATUSES as Product["status"][])))
    .limit(1);
  ...
  const [score, link, reviews, signals] = await Promise.all([
  ...
  return { product, score, link: link[0] ?? null, reviews, signals };
}
...
export async function publicCategories() {
  ...
  const rows = await db
    ...
    .from(categories)
    .innerJoin(products, and(eq(products.categoryId, categories.id), inArray(products.status, PUBLIC_PRODUCT_STATUSES as Product["status"][]), isDemoMode() ? undefined : eq(products.isDemo, false)))
...
export async function productsByIds(ids: string[]) {
  ...
  const rows = await db
    ...
    .from(products)
    .leftJoin(categories, eq(categories.id, products.categoryId))
    .where(and(inArray(products.id, ids), inArray(products.status, PUBLIC_PRODUCT_STATUSES as Product["status"][])));
  return rows.map((r) => ({ ...r.p, categoryName: r.categoryName, categorySlug: r.categorySlug, categoryIcon: r.categoryIcon }));
}
